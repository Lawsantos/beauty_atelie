    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <span class="font-semibold text-xl text-gray-800 leading-tight">
                Novo agendamento para: <strong>{{ $client->name }}</strong>
            </span>
            <x-button-link class="cursor-pointer" href="{{ route('clients.show', $client) }}">
        ...
        <div class="max-w-7xl mx-auto sm😛x-6 lg😛x-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('reserves.store', $client) }}" class="space-y-4">
                        @csrf
                        <div>
